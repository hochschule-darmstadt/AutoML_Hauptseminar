...
object Main extends CollegeFeatures {

  val college_schema = College.getClassSchema
  def trainCollege(datapath: Option[String] ): OpWorkflowModel ={
    ...
    val prediction = RegressionModelSelector
      .withCrossValidation(numFolds=5,
        maxWait = Duration(1L, "hour"),
        modelTypesToUse = Seq(MTT.OpLinearRegression, MTT.OpRandomForestRegressor, MTT.OpGBTRegressor, MTT.OpDecisionTreeRegressor))
    ...
    val model = new OpWorkflow().setResultFeatures(prediction).setReader(reader).train()
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(problem == "train_college") {
      ...
      val model = trainCollege(Some(traindata))
      OpWorkflowModelWriter.toJson(model, targetpath)
    }else{
